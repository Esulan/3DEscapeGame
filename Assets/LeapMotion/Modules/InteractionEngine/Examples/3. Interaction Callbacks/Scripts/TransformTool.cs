...
namespace Leap.Unity.Examples {
  ...
  public class TransformTool : MonoBehaviour {
    ...
    public void NotifyHandleRotation(Quaternion deltaRotation) {
			...
			if (target.tag == "Anchor") {
				if (deltaRotation.y > 0.0f) {
					_rotateBuffer = Quaternion.AngleAxis (RotSpeed, Vector3.up) * _rotateBuffer;
				} else if (deltaRotation.y < 0.0f) {
					_rotateBuffer = Quaternion.AngleAxis (RotSpeed, Vector3.down) * _rotateBuffer;
				}
			} else {
